...
@property (weak) IBOutlet NSTextField *toName;
@property (strong) IBOutlet NSView *onlineTable;
@property (strong) IBOutlet NSTableView *onlineTable;
//user列表
...
- (void)showLogsWithString:(NSString*)str {
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        self.chatView.textStorage = [self.chatView.text stringByAppendingString:newStr];
    });
}
...
- (IBAction)clickSetUserName:(id)sender {
    NSString *msg = [NSString stringWithFormat:@"name:%@",self.userNameField.stringValue] ;
    [self sendMsg:msg];
...
- (IBAction)clickSendMsg:(id)sender {
    if ([self.msgField.stringValue isEqualToString:@""] || ![self.userArray containsObject:self.userNameField.text] || [self.toName.text isEqualToString:self.userNameField.text]) {
